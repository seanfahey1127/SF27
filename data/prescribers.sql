...
select distinct 
	r.specialty_description
from prescriber r 
where r.specialty_description	not in
	(select distinct r2.specialty_description
	from prescriber r2
	join prescription n2
	on r2.npi = n2.npi)
order by r.specialty_description;
	
--     d. **Difficult Bonus:** *Do not attempt until you have solved all other problems!* For each specialty, report the percentage of total claims by that specialty which are for opioids. Which specialties have a high percentage of opioids?

select 
	r.specialty_description,
	sum(case when d.opioid_drug_flag = 'Y' then n.total_claim_count else 0 end) opioid_claims,
	sum(n.total_claim_count) total_claims,
	(sum(case when d.opioid_drug_flag = 'Y' then n.total_claim_count else 0 end) * 100.0 /
	nullif(sum(n.total_claim_count), 0)) opioid_percentage
from prescriber r
join prescription n
on r.npi = n.npi
join drug d 
